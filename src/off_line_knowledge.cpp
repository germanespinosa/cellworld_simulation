...
Off_line_knowledge_map::Off_line_knowledge_map(const cell_world::Graph &world_graph):
    ...
    for (auto &predator_cell:_graph.cells){
        Json_vector<Json_vector<double>> predator_cell_reward;
        Json_vector<Json_vector<unsigned int>> predator_cell_counter;
        for (auto &prey_cell:_graph.cells){
            predator_cell_reward.push_back(Json_vector<double>(_graph[prey_cell].size()));
...
void Off_line_knowledge_map::save_reward(const Cell &prey_cell, const Cell &predator_cell, const Cell &destination, double reward){
...
const Cell &Off_line_knowledge_map::get_destination(const cell_world::Cell &prey_cell, const cell_world::Cell &predator_cell, double probability_best){
    unsigned int predator_cell_index = _graph.cells.find(predator_cell);
    unsigned int prey_cell_index = _graph.cells.find(prey_cell);
...
json_cpp::Json_vector<double> & Off_line_knowledge_map::get_rewards(const Cell &prey_cell, const Cell &predator_cell) {
    unsigned int predator_cell_index = _graph.cells.find(predator_cell);
    unsigned int prey_cell_index = _graph.cells.find(prey_cell);
...
const cell_world::Cell &Off_line_knowledge_map::get_destination(const Model_public_state &state, double probability_best) {
    auto &prey_cell = state.agents_state[0].cell;
    auto &predator_cell = state.agents_state[1].cell;
    return get_destination(prey_cell,predator_cell,probability_best);
}

json_cpp::Json_vector<double> &Off_line_knowledge_map::get_rewards(const Model_public_state &state) {
    auto &prey_cell = state.agents_state[0].cell;
    auto &predator_cell = state.agents_state[1].cell;
    unsigned int predator_cell_index = _graph.cells.find(predator_cell);
    unsigned int prey_cell_index = _graph.cells.find(prey_cell);
