...
Simulation::Simulation(Simulation_parameters &sp) :
    world (sp.world),
    pois (world.create_cell_group(Json_create<Cell_group_builder>(Web_resource::from("cell_group").key(world.name).key("pois").get()))),
    world_graph(sp.world.create_graph()),
    poi_graph(world.create_graph(Json_create<Graph_builder>(Web_resource::from("graph").key(world.name).key("pois").key("astar").get()))),
